...
C_BASE = "#9aa3b0"  # BN-adapt: neutral baseline (recessive, dashed)
C_TENT = "#c0392b"  # red -- the failure mode
C_UNIENT = "#d98a2b"  # orange -- UniEnt family (UniEnt+ = lighter, dashed)
C_NOVA = "#3a3f9c"  # indigo blue -- our method
...
# The method lines shown in the trajectory figures (geometry, confidence,
# absorption): UniEnt+ is dropped -- near-identical to UniEnt, fewer lines.
LINE_ORDER = ["bnadapt", "tent", "unient", "nova"]
ID_C, OOD_C = "#3a6ea5", "#c0392b"  # csID (known, blue) / csOOD (novel, red)
...
def fig_confidence(corruption="gaussian_noise") -> Path:
    ...
    for m in order:
        if m in bands:
            ...
            s = STYLE[m]
            ax.fill_between(
                t, mu - sd, mu + sd, color=s["color"], alpha=0.18, lw=0, zorder=s["z"]
            )
            ax.plot(t, mu, color=s["color"], ls=s["ls"], lw=s["lw"], zorder=s["z"])
...
def fig_distributions(corruption="gaussian_noise") -> Path:
    ...
    rowspec = [("energy", "energy  (higher = novel)"), ("norm", "feature norm")]
    for i, (q, ylab) in enumerate(rowspec):
        for j, m in enumerate(("tent", "nova")):
            ax = axes[i][j]
            d = dump[m]
            idv, oodv = d[f"{q}_id_tT"], d[f"{q}_ood_tT"]
            lo = float(min(idv.min(), oodv.min()))
            hi = float(max(idv.max(), oodv.max()))
...
def fig_openness() -> Path:
    ...
    for m in ("unient", "nova"):
        ys, es = [], []
        for a in alphas:
            mean, sd = _seed_stable(
                [r for r in rows if r["method"] == m and r["alpha"] == a]
            )
            ys.append(mean)
            es.append(sd)
        s = STYLE[m]
...
def fig_tsne(corruption="gaussian_noise", n=1500, seed=0, perplexity=40) -> Path:
    ...
    axes.ravel()[0].legend(
        loc="upper right", fontsize=8.5, frameon=True, markerscale=1.5
    )
    fig.tight_layout()
...
def fig_absorption(corruption="gaussian_noise") -> Path:
    ...
    panels = [("srccos_ood", "novel src-cos"), ("cdist_ood", "novel cdist")]
    plt.rcParams.update({"font.size": 9, "font.family": "sans-serif"})
...
def fig_score_density(corruption="gaussian_noise") -> Path:
    ...
    lo = float(min(id_s.min(), ood_s.min()))
    hi = float(max(id_s.max(), ood_s.max()))
    bins = np.linspace(lo, hi, 40)
    ax.hist(id_s, bins=bins, color=ID_C, alpha=0.55, label="known (csID)", density=True)
    ax.hist(
        ood_s, bins=bins, color=OOD_C, alpha=0.55, label="novel (csOOD)", density=True
    )
...
def fig_pooling() -> Path:
    ...
    data = [
        ("N = 20  (few samples per batch)", load("results/dyn_N20")),
    ...
    for ax, (title, d) in zip(axes, data):
        ...
        t = d["t"]
        ax.plot(
            t, d["perbatch_nll"], color=C_BASE, lw=1.8, label="per-batch fit (UniEnt)"
        )
...
def fig_tradeoff() -> Path:
    ...
    ax.set_xlim(82.2, 85.0)
    ax.set_ylim(86.5, 96.5)
